// Konekt AI Operator edge function — autonomous voice assistant with conversation memory
...
interface Presentation {
  type: "emails" | "documents" | "tasks" | "calendar" | "revenue";
  title: string;
  subtitle?: string;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const conversation: ChatMessage[] = body.conversation || [];
    ...
    const wantsCreate = lowerMsg.includes("cré") || lowerMsg.includes("fait") || lowerMsg.includes("nouveau") || lowerMsg.includes("nouvelle") || lowerMsg.includes("ajoute") || lowerMsg.includes("génère") || lowerMsg.includes("prepare") || lowerMsg.includes("prépare") || lowerMsg.includes("fais");
    ...
    const extractDocInfo = (msg: string) => {
      ...
      // Try to extract client name — look for "pour [Name]" or "client [Name]"
      ...
      // Try to extract title from explicit keywords
      const titleMatch = msg.match(/(?:intitulé|prestation|objet)\s*:?\s*(.+)/i);
      if (titleMatch) info.title = titleMatch[1].trim();

      // Try to extract title from "un devis [title]" or "une facture [title]"
      if (!info.title) {
        const prestationMatch = msg.match(/(?:devis|facture)\s+(?:pour\s+)?(.+?)(?:\s+(?:pour|de|d'un montant|à|au|de\s+\d)\s|$)/i);
        if (prestationMatch && prestationMatch[1] && !prestationMatch[1].match(/^\d/)) {
          const candidate = prestationMatch[1].trim();
          // Don't use client name as title
          if (candidate.toLowerCase() !== (info.client || "").toLowerCase()) {
    ...
    // Check conversation context for pending document info (multi-turn creation)
    const lastAssistantMsg = [...conversation].reverse().find((m) => m.role === "assistant");
    const lastUserMsg = [...conversation].reverse().find((m) => m.role === "user" && m.text !== userMessage);
    const isContinuingDocCreation = lastAssistantMsg?.text.includes("Il me manque") || lastAssistantMsg?.text.includes("quel est le nom du client");
    ...
    if ((wantsDevis || wantsFacture) && wantsCreate) {
      ...
      if (missing.length === 0) {
        ...
        response = `J'ai préparé un ${kind.toLowerCase()} pour ${info.client}, d'un montant de ${info.amount!.toLocaleString("fr-FR")} €. Voici l'aperçu. Voulez-vous que je l'enregistre ?`;
      } else if (missing.length === 3) {
        response = `Avec plaisir ! Pour créer ce ${kind.toLowerCase()}, j'ai besoin de trois informations. D'abord, quel est le nom du client ?`;
      } else {
        ...
        response = `Parfait, j'ai noté : ${provided.join(", ")}. Il me manque encore ${missing.join(" et ")}. Pouvez-vous me les indiquer ?`;
      }
    }
    // === CONTINUE DOCUMENT CREATION (user providing missing info) ===
    else if (isContinuingDocCreation && lastUserMsg) {
      // Try to figure out what kind of doc we were creating from context
      const contextKind = lastAssistantMsg!.text.includes("devis") ? "Devis" : lastAssistantMsg!.text.includes("facture") ? "Facture" : null;
      if (contextKind) {
        // Re-extract from combined context
        const combinedMsg = `${lastUserMsg.text} ${userMessage}`;
        const wantsD = combinedMsg.toLowerCase().includes("devis");
        const wantsF = combinedMsg.toLowerCase().includes("facture");
        const kind = wantsD ? "Devis" : wantsF ? "Facture" : contextKind;

        const info: { kind?: string; client?: string; title?: string; amount?: number } = { kind };
        const amt = parseAmount(userMessage.match(/(\d[\d\s.,]*\d|\d)\s*€?/)?.[1] || amountMatch?.[1]);
        if (amt !== undefined && amt > 0) info.amount = amt;
        const clientMatch = userMessage.match(/([A-ZÉÈÀÂÔÎÛÇ][a-zéèàâôîûç]+(?:\s+[A-ZÉÈÀÂÔÎÛÇa-zéèàâôîûç]+)?)/);
        if (clientMatch) info.client = clientMatch[1];
        // If user message is just a description, use it as title
        if (!info.title && userMessage.length > 3 && !userMessage.match(/^\d/) && !info.client) {
          info.title = userMessage.trim();
        }
        ...
        response = `Pouvez-vous reformuler votre demande ? J'ai perdu le fil de la création du document.`;
      }
    }
    // === REVENUE / CA ===
      ...
      if (revenue.length > 0) {
        ...
        presentation = {
          ...
          subtitle: `${revenue.length} mois de données`,
          items: revenue.map((r) => ({ id: r.month, month: r.month, amount: Number(r.amount), title: r.month, status: `${formatCurrency(r.amount)}` })),
        };
    ...
    // === EMAILS ===
    else if (lowerMsg.includes("mail") || lowerMsg.includes("email") || lowerMsg.includes("courriel") || lowerMsg.includes("boîte") || lowerMsg.includes("lit") && lowerMsg.includes("mail")) {
      if (profile.email_connected) {
        ...
        if (emails.length === 0) {
          ...
          presentation = {
            ...
            subtitle: `${unread.length} non lu(s) sur ${emails.length} au total`,
            items: emails.slice(0, 5).map((e: Record<string, unknown>) => ({
              id: e.id,
              subject: e.subject,
              sender: e.sender,
          ...
          const topEmail = unread[0] || emails[0];
    ...
    // === FACTURES (info) ===
      ...
      if (factures.length > 0) {
        ...
        presentation = {
          type: "documents",
          title: "Vos factures",
          subtitle: `${factures.length} facture(s), ${formatCurrency(total)} au total`,
          items: factures.slice(0, 5).map((f) => ({ id: f.id, title: f.title, client: f.client, amount: f.amount, status: f.status })),
        };
    ...
    // === DEVIS (info) ===
      ...
      if (devis.length > 0) {
        ...
        presentation = {
          type: "documents",
          title: "Vos devis",
          subtitle: `${devis.length} devis en cours`,
          items: devis.slice(0, 5).map((d) => ({ id: d.id, title: d.title, client: d.client, amount: d.amount, status: d.status })),
        };
      ...
      if (lowerMsg.includes("cré") || lowerMsg.includes("ajoute") || lowerMsg.includes("nouvel") || lowerMsg.includes("rappel")) {
        ...
        if (tasks.length === 0) {
          ...
          presentation = {
            type: "tasks",
            title: "Vos tâches",
            subtitle: `${pending.length} en cours sur ${tasks.length}`,
            items: tasks.slice(0, 5).map((t: Record<string, unknown>) => ({ id: t.id, title: t.title, status: t.status, description: t.priority })),
          };
    ...
    // === CONTACTS ===
    else if (lowerMsg.includes("contact") || (lowerMsg.includes("client") && !lowerMsg.includes("facture") && !lowerMsg.includes("devis"))) {
      if (lowerMsg.includes("ajoute") || lowerMsg.includes("cré") || lowerMsg.includes("nouveau")) {
        const nameMatch = userMessage.match(/(?:contact|client)\s+([A-ZÉÈÀÂÔÎÛÇ][a-zéèàâôîûç]+(?:\s+[A-ZÉÈÀÂÔÎÛÇa-zéèàâôîûç]+)?)/);
        const emailMatch = userMessage.match(/[\w.+-]+@[\w-]+\.[\w.-]+/);
        const phoneMatch = userMessage.match(/0[1-9](?:[ .-]?\d{2}){4}/);
        if (nameMatch) {
          const name = nameMatch[1];
          const email = emailMatch ? emailMatch[0] : "";
          const phone = phoneMatch ? phoneMatch[0] : "";
          liveSteps = ["Création du contact…"];
          const { error: contactError } = await supabaseAdmin
            .from("contacts")
            .insert({ user_id: userId, name, email, phone, company: "" });
          if (contactError) {
            response = `Désolé, je n'ai pas pu créer ce contact. Pouvez-vous réessayer ?`;
          } else {
            response = `C'est fait ! J'ai ajouté le contact ${name}${email ? ` (${email})` : ""} à votre liste.`;
            action = { type: "contact_created", label: "Voir les contacts", data: { name } };
          }
        } else {
          response = `Pour ajouter un contact, donnez-moi au moins son nom. Par exemple : « Ajoute le contact Sophie Martin, email sophie@gmail.com ».`;
        }
        ...
        presentation = {
          type: "documents",
          title: "Vos contacts",
          subtitle: `${contacts.length} contact(s) enregistré(s)`,
          items: contacts.slice(0, 5).map((c: Record<string, unknown>) => ({ id: c.id, title: c.name, client: c.company || "", status: c.email || "" })),
        };
    ...
    // === CALENDAR / RDV ===
      ...
      if (events.length > 0) {
        ...
        const upcoming = events.filter((e: { start_time: string }) => new Date(e.start_time) >= new Date()).slice(0, 5);
        if (upcoming.length > 0) {
          presentation = {
            type: "calendar",
            title: "Vos prochains rendez-vous",
            subtitle: `${upcoming.length} événement(s) à venir`,
            items: upcoming.map((e: Record<string, unknown>) => ({
              id: e.id,
              title: e.title,
              status: new Date(e.start_time as string).toLocaleDateString("fr-FR"),
    ...
    return new Response(JSON.stringify({ response, action, live_steps: liveSteps, pending_doc: pendingDoc, presentation }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
